...
@Service
@AllArgsConstructor
public class CredentialsEmailServiceImpl implements CredentialsEmailService {

    private final Logger logger = LoggerFactory.getLogger(CredentialsEmailServiceImpl.class);
    ...
    public void sendManagerCredentials(String to, String password) {
        try {
            ...
            logger.error("Failed to send manager credentials email to '{}': {}", to, e.getMessage(), e);
        }
    }
}